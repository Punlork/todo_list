...
class ErrorInterceptors extends Interceptor {
  @override
  void onRequest(RequestOptions options, RequestInterceptorHandler handler) {
    // options.headers['authorization'] = AppData.apiKey;
...
/*class ErrorQueuedInterceptors extends QueuedInterceptorsWrapper {
  ErrorQueuedInterceptors(this.service);
  final Dio dio = Dio();
      ...
      if (duration < 60) {
        try {
          final newAccessToken = await dio.get(
            AppData.env['baseUrl'] + AppData.refreshToken,
            options: Options(
          ...
          if (newAccessToken.statusCode == 200) {
            ...
            await service
                .saveAccessToken(newAccessToken.data['data']['accessToken']);
            await service
                .saveRefreshToken(newAccessToken.data['data']['refreshToken']);
            return handler.next(options);
          } else {
